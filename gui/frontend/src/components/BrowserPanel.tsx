...
// CDP modifier bitmask: Alt=1, Ctrl=2, Meta=4, Shift=8. Forwarded so shortcuts
// (Cmd/Ctrl+A, +C, …) reach the page as real modified keys.
...
export function BrowserPanel({
    ...
    useEffect(() => {
        ...
        if (interactive)
            client.onClipboard(value => void navigator.clipboard.writeText(value).catch(() => {}))
    ...
    const toFrameCoords = useCallback((e: { clientX: number; clientY: number }) => {
        const canvas = canvasRef.current
    ...
    }, [interactive, toFrameCoords])

    const emitMouse = (action: 'down' | 'up' | 'move', e: React.MouseEvent) => {
        const { x, y } = toFrameCoords(e)
        const ev: InputEvent = { kind: 'mouse', action, x, y, button: BUTTON[e.button] ?? 'left' }
        clientRef.current?.send(ev)
    }
    const emitKey = (action: 'down' | 'up', e: React.KeyboardEvent) => {
        ...
        if (mod && !e.shiftKey && !e.altKey) {
            ...
            if (key === 'v') {
                ...
                if (action === 'down') {
                    ...
                    void navigator.clipboard
                        ...
                        .then(
                            value =>
                                value && clientRef.current?.send({ kind: 'clipboard-write', value })
                        )
                        .catch(() => {})
                }
                return
            ...
            if (key === 'c' || key === 'x') {
                ...
                if (action === 'down')
                    setTimeout(() => clientRef.current?.send({ kind: 'clipboard-read' }), 60)
                return
            }
        }
