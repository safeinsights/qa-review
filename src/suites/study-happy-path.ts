        ...
        },
        {
            ...
            run: ctx =>
                ctx.step(async () => {
                    ...
                    await ctx.page
                        .getByRole('cell', { name: ideMainFile(ctx), exact: true })
                        .waitFor({ state: 'visible' })
                }),
        },
        {
            ...
            run: ctx =>
                ctx.step(async () => {
                    ...
                    await electMainFile(ctx, ideMainFile(ctx))
        ...
        },
        {
            ...
            run: ctx =>
                ctx.step(async () => {
                    ...
                    // it, so remove it and prove the table really dropped it. The row is the
                    ...
                    //
                    // This has to happen BEFORE the upload, not just before the star. The
                    // workspace entry point is `main.R` on qa/staging but `main.r` on
                    // production — the same name round 2 uploads as MAIN_FILE — so once both
                    // are in the table this locator matches two rows and dies on strict mode.
                    // Deleting first leaves exactly one row to name, on every env.
