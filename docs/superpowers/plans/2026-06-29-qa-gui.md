...
- **Phase A (Tasks 1–6): `qatest` CLI + JSON step stream.** The stdout contract the GUI depends on. Usable standalone immediately.
...
qatest/
  bin/
    qatest.ts                    CLI entrypoint: subcommand dispatch
  src/
    cli/
      step-stream.ts             StepEnvelope/ResultEnvelope types + printers (NDJSON)
      args.ts                    tiny arg parser (flags → typed options)
      commands/
        run.ts                   `qatest run` (suite or pr env, --json, --headed)
        login.ts                 `qatest login` (auth, emit session for the skill)
        cleanup.ts               `qatest cleanup` (delete tracked ids)
        codegen.ts               `qatest codegen` (action trace → suite .ts)
    codegen/
  ...
  gui/
    ...
    src-tauri/
      tauri.conf.json
      Cargo.toml
      src/main.rs                spawn qatest/claude/git/gh; stream stdout to webview
    src/
...
# Phase A — `qatest` CLI + JSON step stream
...
## Task 6: The `qatest` CLI entrypoint (run / login / cleanup)

**Files:**
- Create: `bin/qatest.ts`
- Create: `src/cli/commands/run.ts`
- Create: `src/cli/commands/login.ts`
- Create: `src/cli/commands/cleanup.ts`
- Modify: `package.json` (add `"qatest"` script)
...
- [ ] **Step 4: Write `bin/qatest.ts` (dispatch)**
...
- [ ] **Step 5: Add the `qatest` script to package.json**

In `package.json` `scripts`, add:

```json
        "qatest": "tsx bin/qatest.ts",
```

- [ ] **Step 6: Typecheck + verify dispatch (no network)**

Run: `pnpm typecheck && pnpm qatest 2>&1 | head -3`
...
Run: `pnpm qatest run --pr 839 --role admin --suite signin --json 2>&1 | tail -5`
...
```bash
git add bin/qatest.ts src/cli/commands/ package.json && git commit -m "feat: qatest CLI (run/login/cleanup) with --json + --headed"
```
...
## Task 9: `qatest codegen` command (trace file → suite file, typechecked)

**Files:**
- Create: `src/cli/commands/codegen.ts`
- Modify: `bin/qatest.ts` (add `codegen` case)

**Context:** `codegen` reads a trace JSON file, generates the suite source, writes it to `src/suites/<name>.ts`, and typechecks it. If typecheck fails, it deletes the file and exits non-zero so the GUI never pushes broken code. Usage: `qatest codegen --trace <path> [--out src/suites/<name>.ts]`.
...
- [ ] **Step 2: Wire it into `bin/qatest.ts`**
...
JSON
pnpm qatest codegen --trace /tmp/claude/trace.json
```
...
```bash
git add bin/qatest.ts src/cli/commands/codegen.ts src/engine/suite-registry.ts src/engine/run.ts src/cli/index.ts tests/engine/suite-registry.test.ts && git commit -m "feat: qatest codegen + suite discovery (async registry)"
```
...
**Context:** This is the agent prompt. It is not unit-testable; it is validated live. It must instruct Claude to: authenticate via `qatest login`, drive the browser via Claude Code's browser MCP to satisfy the instruction, emit `{"type":"step",...}` NDJSON lines as it goes, record an action trace, ALWAYS run `qatest cleanup` at the end, and (when asked to save) write a trace file and run `qatest codegen`.
...
**Context:** The GUI reads NDJSON lines emitted by `qatest`/the skill. This parser turns a growing buffer of stdout text into typed envelopes, handling partial lines (a chunk may end mid-line). Mirrors `src/cli/step-stream.ts`'s parse but on the consumer side, buffering across chunks.
...
**Context:** The webview needs to (a) start a child process (`pnpm qatest ...` or `claude -p ...`) and (b) receive its stdout lines as events. Use `tauri-plugin-shell` to spawn and forward stdout lines as a Tauri event the frontend listens to. Expose a typed `runProcess(cmd, args)` + `onStdoutLine(cb)` in `ipc.ts`.
...
**Context:** The Suites tab presents environment, role, and suite selectors and a Run button that builds a `RunSpec` for `pnpm qatest run --json --headed ...`. The suite list and env list need to be known to the GUI. To avoid a live engine call at render, the GUI reads them via a tiny `qatest list --json` — but to keep scope tight here, hardcode the known envs (`qa`, `staging`, plus a PR-number input) and fetch suites by spawning `qatest list`. SIMPLER for this task: provide a fixed env set + a PR field, and a suite text input prefilled with `signin`. (A dynamic suite dropdown is Task 16.)
...
## Task 16: Dynamic suite list via `qatest list`

**Files:**
- Create: `src/cli/commands/list.ts`
- Modify: `bin/qatest.ts` (add `list`)
- Modify: `gui/src/components/SuitesTab.tsx` (fetch suites)

**Context:** Replace the suite text input with a real dropdown populated from the engine, so pulled/generated suites appear. Add `qatest list --json` printing `{suites:[{name,description}], envs:[...]}`. The GUI runs it once on mount (capturing stdout) and fills the dropdown.
...
- [ ] **Step 2: Wire `list` into `bin/qatest.ts`**
...
Run: `pnpm qatest list 2>&1 | tail -1`
Expected: a JSON line like `{"suites":[{"name":"signin",...},{"name":"create-study",...}],"envs":["qa","staging"]}`.

- [ ] **Step 4: Use it in SuitesTab — fetch on mount**

Add a one-shot fetch that spawns `qatest list`, accumulates stdout, parses the JSON, and replaces the suite input with a `<select>`. Replace the suite `<input>` and add state + effect:
...
```bash
git add src/cli/commands/list.ts bin/qatest.ts gui/src/components/SuitesTab.tsx && git commit -m "feat: qatest list + dynamic suite dropdown"
```
...
**Context:** Final task wiring the AI mode and git collaboration. Exploratory tab runs `claude -p` with the skill; on a green result it shows `SaveAsSuite`. `SyncButton` runs `git pull`. `SaveAsSuite` invokes a Rust `promote_suite` that branches, runs `qatest codegen`, commits, pushes, and opens a PR via `gh`.
...
async fn promote_suite(app: tauri::AppHandle, cwd: String, name: String, trace_path: String) -> Result<String, String> {
    ...
    run(vec!["pnpm", "qatest", "codegen", "--trace", &trace_path]).await?;
