...
- Modify: `bin/qatest.ts` (add `screencast` to BOOLEANS)
...
- [ ] **Step 6: Add `screencast` to BOOLEANS** — in `bin/qatest.ts`:
...
```bash
git add src/engine/types.ts src/engine/run.ts src/cli/commands/run.ts src/cli/step-stream.ts bin/qatest.ts && git commit -m "feat: --screencast run mode (starts ScreencastServer, emits ws port)"
```
...
Run: `cd /Users/nas/code/si/qatest && pnpm qatest run --json --screencast --role admin --suite create-study --pr 839 2>&1 | tee /tmp/qa-screencast.log`
